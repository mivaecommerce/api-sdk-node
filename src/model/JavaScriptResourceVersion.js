  ...
  constructor(data = {}) {
    ...
    if (!util.isUndefined(this.linkedresources) && util.isArray(this.linkedresources)) {
      for (i = 0, l = this.linkedresources.length; i < l; i++) {
        if (!util.isInstanceOf(this.linkedresources[i], models.JavaScriptResource) && util.isObject(data['linkedresources'][i])) {
          this.linkedresources[i] = new models.JavaScriptResource(this.linkedresources[i]);
        } else if (!util.isInstanceOf(this.linkedresources[i], models.JavaScriptResource)) {
  ...
  toObject() {
    ...
    if (util.isArray(ret['linkedresources'])) {
      for (i = 0, l = ret['linkedresources'].length; i < l; i++) {
        if (util.isInstanceOf(ret['linkedresources'][i], models.JavaScriptResource)) {
